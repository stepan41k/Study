...
GRANT USAGE,
UPDATE ON SEQUENCE z5_student_id_seq TO stud1;

--             ▪ чтение данных представления tek_project с возможностью передачи привилегии;
GRANT
SELECT
    ON TABLE tek_project TO stud1
WITH
GRANT OPTION;
...
REVOKE
SELECT
    (result, project_deadline) ON TABLE z5_project
FROM
    stud1;
...
SELECT
    has_schema_privilege('stud1', 'public', 'USAGE');
...
SELECT
    has_database_privilege('stud1', 'dbstud', 'CONNECT, CREATE');
...
CREATE
OR REPLACE RULE audit_student_insert AS ON INSERT TO z5_student
DO ALSO
INSERT INTO
    audit (username, action, table_name, timestamp)
VALUES
    (current_user, 'INSERT', 'student', now());
...
ADD COLUMN student_rating REAL;

--                 • Создайте политику доступа, чтобы только менторы и администраторы могли видеть рейтинг студентов.
ALTER TABLE z5_student ENABLE ROW LEVEL SECURITY;

CREATE POLICY rating_access_policy ON z5_student FOR
SELECT
    TO mentors,
    administrators USING (true);
...
SET
    student_rating = (
        SELECT
            AVG(result) -- Предполагается, что в таблице project есть столбец 'result' с оценкой
        FROM
            z5_project
        WHERE
            student_id = 1
            AND status = 'completed' -- Фильтруем по завершенным проектам
    )
WHERE
    student_id = 1;
...
CREATE POLICY "stud1_can_view_active_research_projects" ON research_projects FOR
SELECT
    TO stud1 USING (status = 'active');

-- Политика для stud2: полный доступ к исследовательским проектам
CREATE POLICY "stud2_full_access_to_research" ON research_projects FOR ALL TO stud2 USING (true);

-- Политика для stud2: полный доступ к коммерческим проектам
CREATE POLICY "stud2_full_access_to_commercial" ON commercial_projects FOR ALL TO stud2 USING (true);
